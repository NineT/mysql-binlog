...
func (m *LocalMode) Handle(f func(ev *replication.BinlogEvent) bool, a *final.After) {
	...
	for flag {
		...
		default:
			...
			if err := parser.ParseFile(file, int64(m.LatestPos.BinlogPos), handler); err != nil {
				log.Error("parse file error ", err)
				panic(err)
			}

			if strings.Compare(preFile, m.LatestPos.BinlogFile) == 0 {
				// binlog file name is not changed
				panic(fmt.Errorf("binlog file still the same %s", file))
				return
			}
