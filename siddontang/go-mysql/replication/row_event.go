...
import (
	...
	"github.com/mysql-binlog/siddontang/go/hack"
...
type RowsEvent struct {
	...
	// added by an.peng: for row index and row event header and row value
	// row header: remember flag position
	RowsHeader *RowsEventHeader

	//rows: invalid: int64, float64, bool, []byte, string
	Rows [][]interface{}

	// each index in rawData for row values
	RowValueIndex []int
...
func (e *RowsEvent) Decode(data []byte) error {
	// added by an.peng: new row header to remember the right position
	e.RowsHeader = &RowsEventHeader{}
	rh := e.RowsHeader
	//
	...
	// added by an.peng: remember flag position 2 bytes for flag
	rh.FlagsPos = pos
	//
	...
	// added by an.peng: remember header that not changed at any case
	rh.Header = data[:pos]
	//
	...
	start := EventHeaderSize

	var offsets []int
	offsets = append(offsets, start+pos) // start offset
	...
	for pos < len(data) {
		...
		offsets = append(offsets, start+pos) // end position for row
