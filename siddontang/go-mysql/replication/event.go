...
func (h *EventHeader) Encode() []byte {
	bts := make([]byte, EventHeaderSize)
	pos := 0

	binary.LittleEndian.PutUint32(bts[0:], h.Timestamp)
	pos += 4

	bts[pos] = (byte)(EventType(h.EventType))
	pos ++

	binary.LittleEndian.PutUint32(bts[pos:], h.ServerID)
	pos += 4

	binary.LittleEndian.PutUint32(bts[pos:], h.EventSize)
	pos += 4

	binary.LittleEndian.PutUint32(bts[pos:], h.LogPos)
	pos += 4

	binary.LittleEndian.PutUint16(bts[pos:], h.Flags)
